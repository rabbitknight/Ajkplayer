...
public class IndicatorView extends View implements ViewPager.OnPageChangeListener {
    ...
    public IndicatorView(Context context, @Nullable AttributeSet attrs) {
        ...
        valueAnimator1 = ValueAnimator.ofFloat(1.0f, 0).setDuration(5000);
        valueAnimator1.setInterpolator(new LinearInterpolator());
        valueAnimator1.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                setBezierFlexible(0, Float.valueOf(animation.getAnimatedValue().toString()), false);
            }
    ...
    public void setTagCount(int count) {
        this.count = count;
        eachWidth = mWidth / count;
        rects = new ArrayList<>();
        for (int i = 0; i < count; i++) {
            rects.add(new Rect(i * eachWidth, getPaddingTop(), (i + 1) * eachWidth, getPaddingBottom()));
        }
        pos = mViewPager.getCurrentItem();
        bezierCircle.init(rects.get(pos).centerX(), rects.get(pos).centerY(), mHeight > eachWidth ? eachWidth : mHeight);
        edgePoint.setX(0);
    }
    ...
    class BezierCircle {
        ...
        private float finalModX = 1 * 0.7f;
        private float middleModX = 1 * 0.7f / 2;
